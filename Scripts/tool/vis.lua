...
function Visualizer.client_onEquippedUpdate(self, primaryState, secondaryState)
    ...
    if primaryState == 1 and primaryState ~= last_state then
        sm.ACC.debug = not sm.ACC.debug
        sm.gui.displayAlertText("Path logging:" .. bts(sm.ACC.debug), 1.5)
        dprint("sm.ACC.debug=" .. bts(sm.ACC.debug), "info", "vis", "sv")
    end
    last_state = primaryState

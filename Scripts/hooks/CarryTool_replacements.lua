...
local renderablesItemTp = { "$SURVIVAL_DATA/Character/Char_Male/Animations/char_male_tp_item.rend",
    "$SURVIVAL_DATA/Character/Char_Tools/Char_item/char_item_tp_animlist.rend" }
local renderablesItemFp = { "$SURVIVAL_DATA/Character/Char_Male/Animations/char_male_fp_item.rend",
    "$SURVIVAL_DATA/Character/Char_Tools/Char_item/char_item_fp_animlist.rend" }

local renderablesHeavyTp = { "$SURVIVAL_DATA/Character/Char_Male/Animations/char_male_tp_heavytool.rend",
    "$SURVIVAL_DATA/Character/Char_Tools/char_heavytool/char_heavytool_tp_animlist.rend" }
local renderablesHeavyFp = { "$SURVIVAL_DATA/Character/Char_Male/Animations/char_male_fp_heavytool.rend",
    "$SURVIVAL_DATA/Character/Char_Tools/char_heavytool/char_heavytool_fp_animlist.rend" }

local renderablesWormTp = { "$SURVIVAL_DATA/Character/Char_Male/Animations/char_male_tp_toolgorp.rend",
    "$SURVIVAL_DATA/Character/Char_Tools/Char_toolgorp/char_toolgorp_tp.rend" }
local renderablesWormFp = { "$SURVIVAL_DATA/Character/Char_Male/Animations/char_male_fp_toolgorp.rend",
...
local obj_generic_acammorack = sm.uuid.new("3298df18-5437-47d5-9864-e38e3e91f385")
...
function repl_client_onEquippedUpdate(self, primaryState, secondaryState)
    ...
    if (isHarvest or isACCshell) and secondaryState ~= sm.tool.interactState.start then
        ...
        if result.type == "body" then
            ...
            elseif isACCshell then
                ...
                local is_acammorack = shape_uuid == obj_generic_acammorack
                local is_acammo_module = shape_uuid == obj_generic_acammo_module
                if is_breech or is_acbreech or is_ammorack or is_acammo_module or is_acammorack then
                    ...
                    is_right_type = is_right_type or (is_breech and carryUuid == obj_generic_apfsds)
                    is_right_type = is_right_type or (is_ammorack and carryUuid == obj_generic_apfsds)
                    is_right_type = is_right_type or (is_acammorack and carryUuid == obj_generic_acammo)
    ...
    elseif (primaryState == sm.tool.interactState.start and characterShape) or secondaryState == sm.tool.interactState.start then
        ...
        local params = {
            ...
            raycastNormal = result.normalWorld,
            color =
                playerCarryColor
        }
...
function repl_sv_n_dropCarry(self, params)
    ...
    local rotation = sm.quat.angleAxis(pitch, params.camRight) *
        sm.quat.angleAxis(yaw, sm.vec3.new(0, 0, 1)) -- TODO Preview rotation?
        ...
        if not collision then
            if sm.container.beginTransaction() then
                ...
                if sm.container.endTransaction() then
                    sm.unit.createUnit(sm.uuid.new(params.characterShape.characterUuid), spawnPosition,
            ...
            if sm.container.endTransaction() then
                ...
                if sm.item.isBlueprintShape(params.itemA) then
                    ...
                    if params.shapePlacement then
                        local spawnWorldPosition = params.shapePlacement.worldPosition -
                            params.shapePlacement.worldRotation * shapeOffset
                        shape = sm.shape.createPart(params.itemA, spawnWorldPosition, params.shapePlacement
                            .worldRotation, true, true)
                    else
                        local body = sm.body.createBody(spawnPosition, rotation)
                        shape = body:createPart(params.itemA, sm.vec3.new(0, 0, 0), sm.vec3.new(0, -1, 0),
                            sm.vec3.new(1, 0, 0), true)
                    end
                end
                assert(shape)
                ...
                if params.containerA:getSize() > 1 then -- Additional items in carry
                    ...
                    if container == nil then
                        container = interactable:addContainer(0, params.containerA:getSize() - 1,
                            params.containerA:getMaxStackSize())
                    end
